...
frappe.ui.form.on("Branch Sync Config", {
	refresh(frm) {
		if (frm.doc.cloud_url && frm.doc.branch_id) {
			frm.add_custom_button(__("Sync Master Data from Cloud"), function() {
				frappe.call({
					method: "jk_sync.sync.master_data.poll_master_data",
					freeze: true,
					freeze_message: __("Polling Master Data from Cloud..."),
					callback: function(r) {
						if (r.message && r.message.status === "SUCCESS") {
							frappe.msgprint({
								title: __("Master Sync Completed"),
								indicator: "green",
